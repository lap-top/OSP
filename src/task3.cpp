...
void *runner(void *arg)
{
    ...
    std::cout << "Thread elapsed time (length " << globalVec[vec->front()].length() << "): " << elapsed.count() << " s, "
              << "Total Words: " << vec->size() << "\n";
    pthread_exit(vec);
}

void reduce3(std::ofstream &output)
{
...
void map3(std::ifstream &input, std::ofstream &output)
{
    ...
    for (auto const &x : indexMap)
    {
        ...
        pthread_create(&tid[x.first], &attr, runner, (void *)&indexMap[x.first]);
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "ERROR: expected input and output file name as arguments i.e: ./task3 dirty.txt clean.txt" << std::endl;
        return EXIT_FAILURE;
    }
